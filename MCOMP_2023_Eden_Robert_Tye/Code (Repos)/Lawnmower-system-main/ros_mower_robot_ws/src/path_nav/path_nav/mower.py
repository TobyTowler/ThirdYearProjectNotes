...
from .utm_func import utm_bearing, utm_bearing_dist, utm_coords, utm_dist, to_utm
...
class MovePub(Node):
    ...
    def __init__(self):
        ...
        self.subscribition = self.create_subscription(
            RobotSerialReceived,
    ...
    def publish_move(self, turn_speed, move_speed, heading, distance):
        print("Move command: ")
        print("Turning to " + str(heading) + " in direction " + str(turn_speed))
        global turn_complete
        turn_complete = 0
        turn_msg = RobotSendCMD()
        turn_msg.cmd_chr = ord('r')
        turn_msg.arg1 = 100.0 * (turn_speed)
        turn_msg.arg2 = float(abs(heading))
        self.publisher_.publish(turn_msg)
        self.get_logger().info("Sent CMD message: %s" % turn_msg)

        while turn_complete == 0:
            continue

        turn_complete = 0
        move_msg = RobotSendCMD()
        move_msg.cmd_chr = ord('d')
        move_msg.arg1 = 100.0  * move_speed # RPM should be based on time to move 10cm to allow enough time to process
        move_msg.arg2 = float(distance * 1000.0) # metres to mm conversion
        self.publisher_.publish(move_msg)
        self.get_logger().info("Sent CMD message: %s" % move_msg)
        while turn_complete == 0:
            continue
        turn_complete = 0
    ...
    def move_callback(self, msg):
        # Check if == CMD DONE
        self.get_logger().info('Received command from the robot: %s' % msg)
        global turn_complete
        turn_complete = 1
...
class RTKSub(Node):
    ...
    def __init__(self, mower):
        super().__init__('fix_sub')
        self.subscription = self.create_subscription(NavSatFix, '/gps/fix',
                                                    self.listener_callback, 1)
        self.subscription
        self.assigned = mower

    def listener_callback(self, msg):
        # self.get_logger().info("Sent CMD message: %s" % msg)
        gps_points = np.array([[float(msg.latitude), float(msg.longitude)]])
        utm_points = to_utm(gps_points)[0]
        self.assigned.x = utm_points[0]
        self.assigned.y = utm_points[1]
...
class IMUSub(Node):
    ...
    def __init__(self, mower):
        ...
        self.subscription = self.create_subscription(Vector3, '/imu/euler',
                                                    self.listener_callback, 1)
        self.subscription
        self.assigned = mower
...
class Robot():
    ...
    def __init__(self, x, y, target, Q_SIZE, lidar_range, lidar_width,
        ...
        self.lidar = LFCDLaser("/dev/ttyUSB1", 230400)  # lds lidar driver
    ...
    def is_accessible(self, target):
        ...
        vb.append(per_coords)
        for n in self.nogo_polys:
            vb.append(n.exterior.coords)
    ...
    def detect_virtual_boundaries(self, virtual_bounds, target):
        ...
        # Think this may need another loop
    ...
    def detect_objects(self, virtual_bounds, target):
        ...
        for lds in lds_points:
            ...
            if lds[1] > utm_dist([self.x, self.y], target):  # if behind target
                continue
            if lds[1] <= 0.0:  # if no object detected
                continue
        ...
        if len(points) > 0:
            for p in points:
                # Append points for the A*
                self.detected_points.append(p)
            # Only consider the nearest points to avoid
            points = self.find_nearest(points)
            self.detected_points = list(set(self.detected_points))
        ...
        print("Real points: " + str(len(points)))
        # Not the best way to do this, should be seperate, but it will do for now
        virtual_points = self.detect_virtual_boundaries(
            virtual_bounds, target)
        print("Virtual points:" + str(len(virtual_points)))
        # Add virtual obstacles as well
    ...
    def move(self, metres, target):
        ...
        # TODO Change heading to its current to move backwards
        # if not_allowed:
        #     dist = 0.15 
        #     move_speed = -1
        turn_speed = 1
        # Send move command and update position using RTK ROS node
        heading = self.heading - target_loc
        # If heading is negative (left turn, make turn speed negative)
        turn_speed = np.sign(heading)
        if turn_speed == 0:
            turn_speed = 1
        print("Target loc: " + str(target_loc))
        print("current :" + str(self.heading) + " going to: " + str(heading))
        self.move_pub.publish_move(turn_speed, move_speed, heading, dist)
    ...
    def find_target(self, virtual_bounds, target):
        ...
        direction = (360 + self.heading - target_loc) % 360
        if direction > 180:
            direction = 5
        else:
            direction = -5
        ...
        # else:
        #     direction = 2
        # While there are object in the way, turn
        while len(objs) > 0:
            turn_speed = np.sign(direction)
            if turn_speed == 0:
                turn_speed = 1
            # Move to try and find a way with no objects
            # -1 * direction as one uses negative for left but the other doesn't
            self.move_pub.publish_move(turn_speed, 1, direction, 0)
            # Get new objects from new heading
            objs = self.detect_objects([virtual_bounds], target)
            # If facing target and no obstacles, move direct
            if abs(self.heading - target_heading) <= 10 and len(objs) <= 0:
                self.move_pub.publish_move(1, 1, 1, 0.3)
                break
            

    def get_detected_line(self, target, virtual_bounds, heading, centre_line,
    
                          test_shape, current, path):
        ...
        if len(objs) != 0:
            self.move_pub.publish_move(turn_speed, move_speed, heading, 0)
            objs = self.detect_objects([n], path[target])
