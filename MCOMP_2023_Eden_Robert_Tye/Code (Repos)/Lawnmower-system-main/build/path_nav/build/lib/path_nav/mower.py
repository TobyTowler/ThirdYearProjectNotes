...
class MovePub(Node):
    ...
    def publish_move(self, heading, distance):
        ...
        cmd_msg.cmd_chr = ord('d')
        cmd_msg.arg1 = 100.0  # RPM should be based on time to move 10cm to allow enough time to process
        cmd_msg.arg2 = 100.0  # 10 cm
...
class IMUSub(Node):
    ...
    def __init__(self, mower):
        ...
        self.subscription = self.create_subscription(Vector3, 'Robot_Euler_Orientation',
                                                    self.listener_callback, 1)
        self.subscription
        self.assigned = mower
